var changeVertexScale = undefined;
...
var createSphere = function() {
    ...
    var getShader = function() {
        ...
        var fragmentshader = [
            ...
            "vec3 Light0_directionNormalized = vec3(0.0,1.0,0.0);",
    ...
    var grp = osgDB.parseSceneGraph(getSphere());
    var finder = new FindNodeVisitor("Sphere");
    grp.accept(finder);
    //grp = finder.found[0];

    grp.light = new osg.Light();
    grp.light.diffuse = [0.8,0.8,0.8,1];
    grp.light.ambient = [0.9,0.9,0.9,1];
    grp.light.specular = [0.0,0.0,0.0,1];
...
var createSphere2 = function() {
    ...
    var getShader = function() {
        ...
        var fragmentshader = [
            ...
            "LightColor = vec4(0,0,0,0);",
            "//getLightColor(normal);",
            "float base = intensityBase;",
            "float f = pow(turbulence(VertexFrag*vertexScale)*cloudDensity, turbulenceExponent);",
            "if (f < 0.0){",
            " discard;",
            " return;",
            "}",
    ...
    grp.light = new osg.Light();
    grp.light.diffuse = [0.8,0.8,0.8,1];
    grp.light.ambient = [0.5,0.5,0.5,1];

    var stateset = grp.getOrCreateStateSet();
    var prg = getShader();
    stateset.setAttributeAndMode(prg);

    var vertexScale = osg.Uniform.createFloat1(10.0,"vertexScale");
    stateset.addUniform(vertexScale);
    changeVertexScale = function(value) {
        vertexScale.get()[0] = value;
        vertexScale.dirty();
        document.getElementById("VertexScale").innerHTML = value;
    };
    changeVertexScale(10.0);

    var cloudDensity = osg.Uniform.createFloat1(1.0,"cloudDensity");
    stateset.addUniform(cloudDensity);
    changeCloudDensity = function(value) {
        cloudDensity.get()[0] = value;
        cloudDensity.dirty();
        document.getElementById("CloudDensity").innerHTML = value;
    };
    changeCloudDensity(1.0);


    var turbulenceExponent = osg.Uniform.createFloat1(1.0,"turbulenceExponent");
    stateset.addUniform(turbulenceExponent);
    changeTurbulenceExponent = function(value) {
        turbulenceExponent.get()[0] = value;
        turbulenceExponent.dirty();
        document.getElementById("TurbulenceExponent").innerHTML = value;
    };
    changeTurbulenceExponent(1.0);

    var intensityBase = osg.Uniform.createFloat1(1.0,"intensityBase");
    stateset.addUniform(intensityBase);
    changeIntensityBase = function(value) {
        intensityBase.get()[0] = value;
        intensityBase.dirty();
        document.getElementById("IntensityBase").innerHTML = value;
    };
    changeIntensityBase(0.4);
